...
function sortManga(list: MangaEntry[], key: SortKey): MangaEntry[] {
  ...
  switch (key) {
    ...
    case "most-eps":
      return copy.sort((a, b) => b.chapters.length - a.chapters.length);
    case "least-eps":
      return copy.sort((a, b) => a.chapters.length - b.chapters.length);
    case "source":
...
const CoverImage = React.memo(
  ({
    ...
    useEffect(() => {
      getFirstPageUri(uid, firstEp).then(setUri);
    }, [uid, firstEp]);
...
const AdaptiveCard = React.memo(
  ({ item, mode }: { item: MangaEntry; mode: LayoutMode }) => {
      ...
      return (
        <View style={{ marginBottom: 32, paddingHorizontal: 16 }}>
          ...
          <View style={{ marginTop: 14 }}>
            ...
            >
              <Text
                style={{ color: srcColor, fontWeight: "900", fontSize: 12 }}
              >
            ...
            {item.author ? (
              <Text style={{ color: "#38D926", fontSize: 14, marginTop: 2 }}>
                by {item.author}
              </Text>
            ) : null}
            ...
            >
              ...
              />
              <Text
                style={{ color: "#f1f5f9", marginLeft: 8, fontWeight: "600" }}
              >
                {item.chapters.length} Chapters stored locally
              </Text>
            </View>
          </View>
        </View>
      ...
      return (
        ...
        >
          ...
          <View style={{ flex: 1, justifyContent: "space-between" }}>
            <View>
              <Text
                style={{ color: srcColor, fontSize: 10, fontWeight: "900" }}
              >
                {SRC_LABEL[item.source]}
              </Text>
              <Text
            ...
            <Text style={{ color: "#38D926", fontSize: 12, fontWeight: "800" }}>
              {item.chapters.length} Eps
            </Text>
          </View>
        </View>
      );
    ...
    return (
      <View style={{ marginBottom: 20 }}>
        ...
        <View style={{ marginTop: 8, paddingHorizontal: 4 }}>
          <Text style={{ color: srcColor, fontSize: 9, fontWeight: "900" }}>
            {SRC_LABEL[item.source]}
          </Text>
          <Text
            style={{
...
export const LibraryScreen = ({
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#030712" }}>
      <View
        style={{ paddingHorizontal: 16, paddingTop: 52, paddingBottom: 12 }}
      >
        <View
          style={{
            flexDirection: "row",
        ...
        >
          <Text style={{ color: "#f1f5f9", fontSize: 26, fontWeight: "900" }}>
          ...
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            ...
            >
              {(["hero", "list", "grid", "compact"] as LayoutMode[]).map(
                (m) => (
                  <TouchableOpacity
                    key={m}
                    onPress={() => setLayout(m)}
                    style={{
                      padding: 6,
                      borderRadius: 8,
                      backgroundColor: layout === m ? "#1e293b" : "transparent",
                    }}
                  >
                    <MaterialCommunityIcons
                      name={
          ...
          {SORT_OPTIONS.map((opt) => (
            <TouchableOpacity
              ...
              style={{
                ...
                borderRadius: 20,
                marginRight: 8,
                backgroundColor: sortKey === opt.key ? "#38D92615" : "#0a0e17",
                borderWidth: 1,
                borderColor: sortKey === opt.key ? "#38D926" : "#141c2b",
              }}
            >
              <MaterialCommunityIcons
                name={opt.icon as any}
                size={14}
                color={sortKey === opt.key ? "#38D926" : "#475569"}
                style={{ marginRight: 6 }}
              />
              <Text
                style={{
      ...
      <FlatList
        ...
        columnWrapperStyle={
          numColumns > 1
            ? { justifyContent: "flex-start", gap: 12, paddingHorizontal: 16 }
            : null
        }
          ...
          <TouchableOpacity
            ...
            onLongPress={() =>
              Alert.alert("Options", item.name, [
                ...
                {
                  text: "Delete Library Entry",
                  style: "destructive",
                  onPress: () => onDeleteManga(item.uid),
                },
              ])
            }
          >
